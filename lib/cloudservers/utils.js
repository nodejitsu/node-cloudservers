  ...
  try {
    request(serverOptions, function (err, res, body) {
      ...
      if (Object.keys(failCodes).indexOf(statusCode) !== -1) {
        if (callback) {
          var faultError = new Error('Rackspace Error (' + statusCode + '): ' + failCodes[statusCode]);
          try { 
            faultError.fault = JSON.parse(body).cloudServersFault;
          }
          catch (ex) {
            faultError.fault = { message: 'Unknown fault. Cloudservers responded with invalid JSON.' };
          }
          finally {
            callback(faultError);
          }
...
utils.serverUrl = function () {
  ...
  return [cloudservers.config.serverUrl].concat(args).join('/');
};